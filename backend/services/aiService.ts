...
async function verifyReturnWindowWithOcr(
  ...
  try {
    ...
    if (expected.expectedSoldBy) {
      ...
      // Fallback: require ALL core words (≥3 chars) to appear, not just any one
      if (!soldByMatch) {
        const coreWords = sellerNorm.split(/\s+/).filter(w => w.length >= 3);
        if (coreWords.length > 0) {
          soldByMatch = coreWords.every(w => lower.includes(w));
        }
      }
    }
      ...
      try {
        const response = await withModelTimeout(ai.models.generateContent({
          ...
          contents: [
            { inlineData: { mimeType, data: payload.imageBase64.split(',')[1] || payload.imageBase64 } },
            { text: [
              `RETURN WINDOW / DELIVERY SCREENSHOT VERIFICATION — STRICT COMPARISON MODE`,
              ``,
              `You MUST compare the screenshot against EXACT expected values below.`,
              `DEFAULT TO FALSE for any field you cannot confidently verify. Never assume a match.`,
              ``,
              `  Expected Order ID: ${payload.expectedOrderId}`,
              `  Expected Product Name: ${payload.expectedProductName}`,
              ...
              `1. Find the ORDER ID in the screenshot and compare character-by-character to "${payload.expectedOrderId}". If NOT found or different → orderIdMatch=false.`,
              `2. Find the PRODUCT NAME and compare to "${payload.expectedProductName}". ALL significant words (brand, model, type, variant name, fragrance/flavor, size, color, capacity) must match. CRITICAL: The same brand often has many DIFFERENT products with very similar names (e.g. "Arabian Aroma Old Money Eau de Parfum 50ml" vs "Arabian Aroma Sovage Perfume 50ml" — these are DIFFERENT products even though both are Arabian Aroma perfumes). If even ONE distinguishing word differs, set productNameMatch=false. A different product MUST be false. If the product name is not visible → productNameMatch=false.`,
              `3. Find the GRAND TOTAL / AMOUNT and compare to ₹${payload.expectedAmount} (±₹1 tolerance only). If NOT found or different → amountMatch=false.`,
  ...
  try {
    ...
    const extractAmounts = (text: string, detectedOrderId?: string | null) => {
      ...
      const getGenericLabelWeight = (line: string): number => {
        ...
        if (/selling\s*price|item\s*price/i.test(line)) return 1; // Low weight — only used as last resort
        return 1;
      };
        ...
        contents: [
          {
            text: [
              ...
              '   - Look for labels: "Grand Total", "Amount Paid", "You Paid", "Order Total", "Total Amount", "Total amount", "Payable", "Net Amount", "Amount Due", "To Pay", "Amount Payable", "Bill Total", "Net Payable".',
              ...
              '   - MYNTRA: Use "Total" or "Amount Payable" from the bottom of price breakdown. IGNORE "MRP", "Discount on MRP".',
              '   - MEESHO: Use "Total Amount" or "You Paid" or "Total Payable". IGNORE "Supplier Price", "Discount".',
              '   - BLINKIT/ZEPTO/SWIGGY: Use "Bill Total" or "Grand Total" or "Amount Paid" or "To Pay".',
              '   - NYKAA: Use "Amount Payable" or "Grand Total". IGNORE "MRP", "Savings".',
              '   - AJIO: Use "Order Total" or "Amount Payable". IGNORE "Product MRP".',
              '   - GENERIC: For ANY platform, prefer: "Grand Total" > "Total Amount" > "Amount Paid" > "You Paid" > "Payable" > "Total" at BOTTOM of price section.',
              ...
              '   - Amount format: ₹1,408 → 1408, ₹41,990.00 → 41990, ₹6,003 → 6003, ₹1,04,990 → 104990 (Indian lakh format). Return as plain number (no comma, no ₹).',
              '   - INDIAN NUMBER FORMAT: ₹1,04,990 = 104990. ₹10,04,990 = 1004990. ₹99,999 = 99999. Always strip commas and .00 decimal.',
              '   - If multiple amounts are visible, pick the FINAL/TOTAL amount the customer paid — usually the LAST and LARGEST in the price section (not the struck-through MRP).',
              '',
      ...
      const response = await withModelTimeout(ai.models.generateContent({
        ...
        contents: [
          ...
          { text: [
            ...
            '   - MEESHO: Look for "Total Amount" or "You Paid" or "Total Payable". The price section is usually at the bottom. IGNORE "Supplier Price" and "Discount".',
            '   - BLINKIT/ZEPTO/SWIGGY: Look for "Bill Total" or "Grand Total" or "Amount Paid" or "To Pay". Often displayed prominently at the top or bottom.',
            '   - MYNTRA: Look for "Total" or "Amount Payable" at the BOTTOM of the price breakdown. IGNORE "MRP" and "Discount on MRP". The order ID is near "Order ID" label.',
            '   - NYKAA: Look for "Amount Payable" or "Total" or "Grand Total". IGNORE "MRP" and "Savings".',
            '   - AJIO: Look for "Order Total" or "Amount Payable" or "Total Amount". IGNORE "Product MRP".',
            '   - CROMA: Look for "TOTAL" or "Amount Payable" or "Grand Total".',
            '   - TATA CLIQ: Look for "Order Total", "Total" or "Amount Paid".',
            '   - LENSKART: Look for "Total" or "Amount to Pay" or "Net Amount".',
            '   - PHARMEASY/1MG: Look for "Total Amount" or "Net Payable" or "Amount Payable".',
            '   - SHOPSY: Same as Flipkart — look for "Total amount".',
            '   - GENERIC: For ANY platform, prefer these label names in order: "Grand Total" > "Total Amount" > "Amount Paid" > "You Paid" > "Payable" > "Net Amount" > "To Pay" > "Total" at BOTTOM of price section.',
            ...
            '   - Return the amount as a plain number WITHOUT the ₹ symbol. Example: for ₹10,048 return 10048. For ₹604.00 return 604. For ₹1,408 return 1408. For ₹12,999 return 12999. For ₹1,04,990 return 104990.',
            '   - INDIAN NUMBER FORMAT: ₹1,04,990 = 104990 (lakh format). ₹10,04,990 = 1004990. ₹99,999 = 99999. Always strip commas and .00 decimal.',
            '   - If multiple amounts are visible, pick the FINAL/TOTAL amount that the customer actually paid. This is usually the LAST and LARGEST amount in the price breakdown section (not the MRP which is struck through).',
            '',
            ...
            '   - MYNTRA: Brand name line + product type line should be combined. E.g., "H&M" + "Men Slim Fit Pure Cotton T-Shirt" → "H&M Men Slim Fit Pure Cotton T-Shirt". Look near the product image.',
            '   - MEESHO: Product title near the product image. Often in bold. E.g., "Men Regular Fit Polycotton Casual Shirt" or "Women\'s Synthetic Embroidered Kurta Set".',
            '   - BLINKIT/ZEPTO/BIGBASKET: Product name with weight/quantity (e.g., "Amul Butter 500g", "Tata Salt 1kg").',
            '   - NYKAA/PURPLLE: Beauty product with brand + product + size (e.g., "Maybelline Fit Me Foundation 115 Ivory 30ml").',
            '   - AJIO: Brand + product type + details. E.g., "PUMA Solid Running Shoes" or "ALLEN SOLLY Striped Regular Fit Shirt".',
            '   - CROMA: Product title from the electronics category. E.g., "Samsung Galaxy S24 Ultra (Titanium Gray, 256 GB)".',
            '   - TATA CLIQ: Full product title with brand and specifications.',
            '   - LENSKART: Frame name with model details. E.g., "Vincent Chase VC E13984 Black Full Frame Eyeglasses".',
