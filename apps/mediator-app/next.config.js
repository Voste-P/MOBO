...
const nextConfig = {
    ...
    return [
      {
        ...
        headers: [
          ...
          { key: 'Content-Security-Policy', value: "default-src 'self'; script-src 'self' 'unsafe-inline' https://vercel.live; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https:; font-src 'self' data:; connect-src 'self' https: wss:; frame-ancestors 'none'" },
        ],
      },
    ];
  },
  async rewrites() {
    ...
    if (!process.env.NEXT_PUBLIC_API_PROXY_TARGET && process.env.VERCEL) {
      console.warn('[mediator-app] ⚠️  NEXT_PUBLIC_API_PROXY_TARGET is not set — API calls will fail. Set it to your backend URL in Vercel project settings (all environments).');
    }
    return [
      {
        source: '/api/:path*',
